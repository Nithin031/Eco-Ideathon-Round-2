...
### 6. Distance Verification (NEW in Round 2)

For each detected solar panel, the pipeline computes:

- **Euclidean distance (m)** from the **centroid of the selected panel** to the **query coordinate (lat, lon)**
- This metric validates spatial accuracy and is used for RMSE evaluation
- Distance = 0 indicates the panel centroid is exactly at the query point
- Larger distances may indicate geocoding jitter or off-target detection

**Formula:**
```
dx_m = (centroid_x - image_center_x) × meters_per_pixel
dy_m = (centroid_y - image_center_y) × meters_per_pixel
euclidean_distance_m = √(dx_m² + dy_m²)
```

**Note:** Image center corresponds to the input (lat, lon) coordinate.
---
...
```json
{
  ...
  "euclidean_distance_m_est": 2.34,
  "qc_status": "VERIFIABLE",
  "reason_codes": ["high_model_confidence", "rectilinear_array"],
  "bbox_or_mask": {
    "type": "Polygon",
    "coordinates": [...]
  },
  "image_metadata": {
    "source": "Google Static Maps (640px)",
    "capacity_assumption_wp_per_m2": 190,
    "panel_area_assumption_m2": 1.9,
