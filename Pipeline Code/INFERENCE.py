...
class PipelineConfig:
    # -----------------------------------------------------
    # Image & Geospatial Parameters
    # -----------------------------------------------------
    IMG_SIZE_PX = 640              # Must match YOLO imgsz
    DEFAULT_ZOOM = 20              # Rooftop-level detail
    IMG_SCALE = 1                  # Non-retina imagery (must match fetcher)
    JITTER_METERS = 10              # Allowed geocoding jitter
    REQUEST_TIMEOUT_SECONDS = (5, 10)

    # -----------------------------------------------------
    # Detection Thresholds
    # -----------------------------------------------------
    CONF_THRESHOLD = 0.25           # Used at inference; F1 tuned later
    MIN_POLY_AREA_PX = 50           # Filters tiny false-positive blobs
    ...
    # -----------------------------------------------------
    # PV Geometry & Capacity Assumptions
    # -----------------------------------------------------
    WP_PER_M2 = 190                 # Conservative crystalline silicon assumption
    PANEL_AREA_M2 = 1.9             # Typical rooftop panel area

    # -----------------------------------------------------
    # Confidence Calibration
    # -----------------------------------------------------
    CALIB_AREA_SATURATE_M2 = 5.0     # Confidence saturates beyond this area
    CONFIDENCE_CALIBRATION_TYPE = "area_weighted_heuristic"

    # -----------------------------------------------------
    # Buffer Search (as per problem statement)
    # -----------------------------------------------------
    BUFFER_SEARCH_SQFT = [1200, 2400]

    # -----------------------------------------------------
    # Distance Estimation Method (for RMSE auditability)
    # -----------------------------------------------------
    DISTANCE_METHOD = "centroid_to_query_point"
...
def estimate_euclidean_distance_m(
    poly: Polygon | None,
    img_width: int,
    img_height: int,
    ...
    """
    Estimates Euclidean distance (meters) between the centroid of the selected
    solar panel polygon and the image center (query coordinate).
    """
    poly_single = ensure_single_polygon(poly)
    if poly_single is None or poly_single.is_empty:
        return 0.0

    centroid = poly_single.centroid

    # Image center corresponds to queried (lat, lon)
    cx = img_width / 2.0
    cy = img_height / 2.0
    ...
    return round(math.sqrt(dx_m**2 + dy_m**2), 2)
...
def basic_qc(img: Image.Image) -> dict:
    ...
    if w < PipelineConfig.IMG_SIZE_PX or h < PipelineConfig.IMG_SIZE_PX:
    ...
    if contrast < 15:   # tuned for satellite imagery
...
def qc_score(qc: dict) -> float:
    score = 0.0

    if "LOW_RESOLUTION" in qc["flags"]:
        score += 10.0

    if "CLOUD_GLARE" in qc["flags"]:
        score += 8.0

    if "DARK_SHADOW" in qc["flags"]:
        score += 6.0

    if "LOW_CONTRAST" in qc["flags"]:
        score += 12.0  # strongest penalty

    score += abs(qc["brightness"] - 120) / 50.0
...
def fetch_google_static_maps(lat: float, lon: float, api_key: str) -> tuple[Image.Image, str]:
    """
    Fetches a 640x640 satellite image using Google Static Maps.
    Image resolution is fixed to match model training resolution.
    """
    if not api_key:
        raise RuntimeError("GOOGLE_API_KEY_NOT_SET")

    # Google Static Maps: max 640x640 per scale unit
    ...
    headers = {
        "User-Agent": "SolarVerificationPipeline"
    }
    ...
    return img, "Google Static Maps (640px)"

def fetch_esri_world_imagery(lat: float, lon: float) -> tuple[Image.Image, str]:
    """
    Fetches a 640x640 satellite image from ESRI World Imagery service.
    """
    try:
        ...
        headers = {"User-Agent": "SolarVerificationPipeline"}
        ...
        content_type = response.headers.get("Content-Type", "")
        if "image" not in content_type:
            raise RuntimeError(f"ESRI_NON_IMAGE_RESPONSE: {content_type}")

        img = Image.open(BytesIO(response.content)).convert("RGB")

        if img.size != (PipelineConfig.IMG_SIZE_PX, PipelineConfig.IMG_SIZE_PX):
            raise RuntimeError(
                f"ESRI_IMAGE_SIZE_MISMATCH: got {img.size}"
            )
...
def get_best_image(original_lat: float, original_lon: float, api_key: str | None) -> dict | None:
    ...
    """
    candidates = []
    jitter_attempts = generate_jitters(
        original_lat, original_lon, PipelineConfig.JITTER_METERS
    )

    for i, (jlat, jlon) in enumerate(jitter_attempts):
        log(
            "FETCH",
            f"Attempt {i+1}/{len(jitter_attempts)} for "
            f"({original_lat:.4f},{original_lon:.4f}) "
            f"at jitter ({jlat:.4f},{jlon:.4f})"
        )

        # --- compute jitter offset in meters (audit + RMSE metric) ---
        dx_m = (jlat - original_lat) * 110574
        dy_m = (jlon - original_lon) * 111320 * math.cos(math.radians(original_lat))
        jitter_offset_m = math.sqrt(dx_m**2 + dy_m**2)
        ...
        if api_key:
            try:
                ...
                candidates.append({
                    ...
                    "score": qc_score(qc),
                    "source": src,
                    "jitter_offset_m": round(jitter_offset_m, 2)
                })
    ...
    log(
        "FETCH",
        f"Selected image from {best['source']} | "
        f"QC score={best['score']:.2f} | "
        f"jitter_offset_m={best['jitter_offset_m']}"
    )
...
def pixel_xy_to_latlon(x_px, y_px, img_width, img_height, center_lat, center_lon, m_per_px):
    """
    Convert pixel (x,y) -> lat, lon
    image center pixel = query coordinate (center_lat, center_lon)
    x to right = +lon, y downward = +south (lat decreases)
    """
    cx = img_width / 2.0
    cy = img_height / 2.0
    ...
    lat = center_lat - (dy_m / 110574.0)
    lon = center_lon + (dx_m / (111320.0 * math.cos(math.radians(center_lat))))

    return lon, lat  # GeoJSON order

def polygon_pixels_to_geojson_latlon(poly, img_w, img_h, center_lat, center_lon, m_per_px):
    if poly is None:
        return None

    poly_single = ensure_single_polygon(poly)
    if poly_single is None:
        return None

    exterior = []
    for x, y in poly_single.exterior.coords:
        lon, lat = pixel_xy_to_latlon(
            x, y, img_w, img_h, center_lat, center_lon, m_per_px
        )
...
def save_buffer_image(img_np: np.ndarray, center_pt: Point, radius_px: float, out_path: str):
    ...
    cv2.circle(vis_img, (int(center_pt.x), int(center_pt.y)), int(radius_px), (0, 255, 255), 2) # Yellow circle
    cv2.imwrite(out_path, vis_img)
...
class SolarVerificationPipeline:
    ...
    def process(self, row: pd.Series) -> dict:
        ...
        # Euclidean distance (meters) between original coord and
        # the selected jittered coordinate (for evaluation metric)
        # ---------------------------------------------------------
        dx_m = (effective_lat - original_lat) * 110574
        dy_m = (effective_lon - original_lon) * 111320 * math.cos(math.radians(original_lat))
        euclidean_distance_m = round(math.sqrt(dx_m**2 + dy_m**2), 2)
        ...
        for buf_sqft in PipelineConfig.BUFFER_SEARCH_SQFT:
            ...
            for panel_candidate in detected_panels:
                try:
                    ...
                    overlap_ratio = intersection_area / panel_candidate["poly"].area
        ...
        # Euclidean distance (meters) from PV centroid to query point
        euclidean_distance_m = estimate_euclidean_distance_m(
            pv_polygon,
            img_width,
            img_height,
            m_per_px
        ...
        # =======================
        # Draw SELECTED panel (GREEN) — existing logic
        # =======================
        ...
        # =======================
        # Draw NON-SELECTED panels (RED) — REQUIRED FIX
        # =======================
        for panel_candidate in detected_panels:
            if panel_candidate is chosen_panel_info:
                continue  # skip selected panel

            poly_red = ensure_single_polygon(panel_candidate["poly"])
            if poly_red is None:
                continue

            try:
                red_pts = (
                    np.array(poly_red.exterior.coords)
                    .astype(np.int32)
                    .reshape(-1, 1, 2)
                )
                ...
                log("DRAW_WARN", f"Failed to draw non-selected panel: {e}")
        ...
        final_record = {
            ...
            "image_metadata": {
                "source": image_source,
                "capacity_assumption_wp_per_m2": PipelineConfig.WP_PER_M2,
                "panel_area_assumption_m2": PipelineConfig.PANEL_AREA_M2,
                "confidence_calibrated": True,
                "confidence_method": "area_weighted_heuristic"
            }
        }
